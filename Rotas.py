...
# -------------------------------
# Geocodificação com OpenCage (ajustada)
# -------------------------------
...
if uploaded_file is not None:
    ...
    # -------------------------------
    # Filtros reintegrados
    # -------------------------------
    st.sidebar.header("📌 Filtros de rota")
    dia = st.sidebar.selectbox("Selecione o dia", df["Dias de visita"].unique())
    motorista = st.sidebar.selectbox("Selecione o motorista", df["Motorista"].unique())
    veiculo = st.sidebar.selectbox("Selecione o veículo", df["Veículo"].unique())

    dados_filtrados = df[
        (df["Dias de visita"].str.contains(dia)) &
