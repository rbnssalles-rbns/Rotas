...
# Geocodificação automática via OpenCage
...
# Cálculo de rota DE → PARA com ORS
def calcular_rota(coords, tempo_atendimento=1800, jornada=28800):
    url = "https://api.openrouteservice.org/v2/directions/driving-car"
    headers = {"Authorization": ORS_KEY, "Content-Type": "application/json"}
    body = {
        "coordinates": [[lon, lat] for lat, lon in coords if lat and lon],
        "format": "geojson",
    ...
    props = rota.get("segments", [{}])[0]
    distancia_km = props.get("distance", 0) / 1000
    tempo_seg = props.get("duration", 0)

    # Decodificação segura da geometria
    geometry = rota.get("geometry", {})
    if isinstance(geometry, str):
        geometry = convert.decode_polyline(geometry)
...
# 🔍 Teste rápido ORS antes de processar Excel
st.write("🔍 Teste rápido ORS:")
coords_teste = [(-3.7347, -38.5268), (-3.7241, -38.5232)]
tabela_teste, capacidade_teste, geom_teste = calcular_rota(coords_teste)
st.write("Resultado do teste:", tabela_teste, capacidade_teste)
...
if uploaded_file is not None:
    ...
    # Geocodificação automática com progresso
    ...
    # Sinalização de endereços não encontrados
