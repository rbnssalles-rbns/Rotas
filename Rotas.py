...
def verificar_conectividade(coords):
    ...
    for i in range(len(coords) - 1):
        ...
        if "routes" in response:
            conectados.append((lat1, lon1))
        else:
            st.warning(f"⚠️ Sem rota entre Cliente {i} e Cliente {i+1} — ponto removido.")
            desconectados.append((lat1, lon1))

    # Adiciona o último ponto se ainda houver conectividade
    if conectados and conectados[-1] != coords[-1]:
        conectados.append(coords[-1])

    st.write("✅ Pontos conectados:", len(conectados))
    st.write("❌ Pontos removidos:", len(desconectados))
    return conectados
...
def calcular_rota(coords, tempo_atendimento=1800, jornada=28800):
    ...
    if "routes" not in response or not response["routes"]:
        st.error("❌ ORS não retornou rotas.")
        st.write("🔎 Resposta completa do ORS:", response)
        return [], {}, []
    ...
    coords_geom = geometry['coordinates']
    ...
    return tabela_de_para, capacidade, coords_geom
...
if uploaded_file is not None:
    ...
    if st.button("Atualizar rotas"):
        ...
        if coords_original:
            ...
            coords_validos = verificar_conectividade(coords_original)

            if len(coords_validos) >= 2:
                # Rota Original
                tabela_orig, capacidade_orig, rota_orig = calcular_rota(coords_validos)
                st.subheader("Rota Original")
                st.write("➡️ DE → PARA")
                st.dataframe(pd.DataFrame(tabela_orig))
                st.write("📊 Capacidade")
                st.dataframe(pd.DataFrame([capacidade_orig]))
                mostrar_rota(coords_validos, rota_orig, "Rota Original", [0, 0, 255])

                # Rota Otimizada (exemplo simples: invertida)
                tabela_otim, capacidade_otim, rota_otim = calcular_rota(coords_validos[::-1])
                st.subheader("Rota Otimizada")
                st.write("➡️ DE → PARA")
                st.dataframe(pd.DataFrame(tabela_otim))
                st.write("📊 Capacidade")
                st.dataframe(pd.DataFrame([capacidade_otim]))
                mostrar_rota(coords_validos[::-1], rota_otim, "Rota Otimizada", [255, 0, 0])
            else:
                st.error("Nenhuma rota conectada encontrada. Verifique endereços.")
        else:
